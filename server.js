...
io.on('connection', (socket) => {
    ...
    // Передача голосов клиентам
    socket.on('voteMode', mode => { 
        let l = lobbies[socket.lobbyId]; 
        if(l && l.state === 'voting_mode' && l.votesMode[mode]!==undefined) {
            l.votesMode[mode]++; 
            io.to(l.id).emit('voteUpdate', { type: 'mode', votes: l.votesMode });
        } 
    });
    socket.on('voteDiff', diff => { 
        let l = lobbies[socket.lobbyId]; 
        if(l && l.state === 'voting_diff' && l.votesDiff[diff]!==undefined) {
            l.votesDiff[diff]++; 
            io.to(l.id).emit('voteUpdate', { type: 'diff', votes: l.votesDiff });
        } 
    });
    ...
    // Синхронизация движения монстра-игрока
    socket.on('monsterMove', data => { 
        let l = lobbies[socket.lobbyId]; 
        if(l) { 
            if(l.players[socket.id]) {
                l.players[socket.id].x = data.x;
                l.players[socket.id].z = data.z;
            }
            socket.to(l.id).emit('monsterUpdate', data); 
        } 
    });
