...
function startMatchLoops(room) {
    ...
    room.matchLogicInterval = setInterval(() => {
        ...
        // ЛОГИКА ТЕЛЕПОРТА (ПОЯВЛЕНИЕ КНОПКИ) - ИЗМЕНЕНА ДИСТАНЦИЯ НА 26 БЛОКОВ
    ...
    socket.on('requestMonsterTeleport', () => {
        ...
        if (room && room.gameState === 'playing' && room.monsterId === socket.id && room.teleportCheckTimer >= 5.0) {
            ...
            if(humanTargets.length > 0) {
                ...
                for(let z = Math.max(1, pg.z - 5); z <= Math.min(MAP_SIZE-2, pg.z + 5); z++) {
                    for(let x = Math.max(1, pg.x - 5); x <= Math.min(MAP_SIZE-2, pg.x + 5); x++) {
                        if(room.mapData[z][x] === 0) {
                            let dist = Math.hypot(x - pg.x, z - pg.z);
                            // РАДИУС ИЗМЕНЕН НА 4-5 БЛОКОВ
                ...
                if(validSpots.length > 0) {
                    ...
                    room.monster.x = randomHuman.x - 16; // если нет свободных рядом, кидает чуть дальше
