...
function evaluateModeVoting(room) {
    ...
    room.votingTimer = 8; // Сделали старт быстрее
...
function runDifficultyVoting(room) {
    if (room.chosenMode === 'monster') {
        // Пропускаем голосование в режиме монстра, рандом без 'easy'
    ...
    room.votingTimer = 15; // Сделали старт быстрее
...
function startMatchLoops(room) {
    ...
    room.matchLogicInterval = setInterval(() => {
        ...
        // ЛОГИКА ТЕЛЕПОРТА (ПОЯВЛЕНИЕ КНОПКИ)
        if (room.chosenMode === 'monster' && room.monsterId) {
            ...
            for(let id in room.players) {
                ...
                if(!p.isDead && !p.isMonsterPlayer) {
                    ...
                    if (d <= 20) { allFar = false; break; } // В радиусе 20 блоков есть игрок
                }
            }

            if(allFar) {
                room.teleportCheckTimer += 0.5; // Тик раз в полсекунды
        ...
        // ЛОГИКА УБИЙСТВА ПРИ ПРИБЛИЖЕНИИ
        if (room.chosenMode === 'monster' && room.monsterId) {
            for(let id in room.players) {
                let p = room.players[id];
                if(!p.isDead && !p.isMonsterPlayer) {
        ...
        // АВТОНОМНЫЙ ИИ ДЛЯ ОБЫЧНОГО ОНЛАЙН РЕЖИМА
        if (room.chosenMode === 'normal') {
            let target = null;
            
            // Фиксируем одну цель и не отпускаем, пока она не умрёт
            ...
            if (target) {
                let mdx = target.x - room.monster.x;
                let mdz = target.z - room.monster.z;
                let dist = Math.hypot(mdx, mdz);
                
                if (dist > 0.5) {
                    let speed = monsterSpeeds[room.difficulty] * 0.5;
                    let moveX = (mdx / dist) * speed;
                    let moveZ = (mdz / dist) * speed;
                    
                    if(!checkServerWallCollision(room.mapData, room.monster.x + moveX, room.monster.z, 0.4)) {
                        room.monster.x += moveX;
                    }
                    if(!checkServerWallCollision(room.mapData, room.monster.x, room.monster.z + moveZ, 0.4)) {
                        room.monster.z += moveZ;
                    }
    ...
    socket.on('requestMonsterTeleport', () => {
        ...
        if (room && room.gameState === 'playing' && room.monsterId === socket.id && room.teleportCheckTimer >= 5.0) {
            ...
            if(humanTargets.length > 0) {
                ...
                // Ищем пустые клетки вокруг игрока (отступ 1.5 - 2.5 блоков сетки = 6-10 метров)
                let validSpots = [];
                let pg = {x: Math.floor(randomHuman.x/BLOCK_SIZE), z: Math.floor(randomHuman.z/BLOCK_SIZE)};
                for(let z = Math.max(1, pg.z - 3); z <= Math.min(MAP_SIZE-2, pg.z + 3); z++) {
                    for(let x = Math.max(1, pg.x - 3); x <= Math.min(MAP_SIZE-2, pg.x + 3); x++) {
                ...
                if(validSpots.length > 0) {
                    ...
                    // Страховка (если вдруг генератор подвёл)
                    room.monster.x = randomHuman.x - 6;
                    room.monster.z = randomHuman.z - 6;
                }
