...
function finalizeAndStartMatch(room) {
    ...
    let idx = 0;
            ...
            // Выбираем одну общую точку спавна для всех выживших
    let baseSpot = spots.length > 0 ? spots[0] : pSpot; 

    for(let id in room.players) {
        let p = room.players[id];
        if (p.isMonsterPlayer) {
            // Монстр спавнится на своей позиции (обычно вдали от выживших)
            p.x = room.monster.x;
            p.z = room.monster.z;
        } else {
            // Спавним выживших в круг вокруг одной точки, чтобы они видели друг друга
            let angle = idx * (Math.PI / 2); // Смещаем каждого следующего по кругу
            let spreadRadius = BLOCK_SIZE * 0.6; // Расстояние между игроками (чуть больше полблока)
            
            p.x = baseSpot.x * BLOCK_SIZE + Math.cos(angle) * spreadRadius;
            p.z = baseSpot.z * BLOCK_SIZE + Math.sin(angle) * spreadRadius;
        }
        p.isDead = false;
