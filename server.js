...
let playerVotes = {}; // чтобы один раз можно было голосовать
...
function manageLobbyTimer() {
    ...
    if (pCount >= 2 && gameState === 'waiting' && !lobbyTimer) {
        countdown = 20; // 20 секунд до начала
        ...
        // Если кто-то вышел и остался 1 игрок, сбрасываем таймер
        clearInterval(lobbyTimer);
        lobbyTimer = null;
        countdown = 20;
...
function startGame() {
    ...
    // Единая точка спавна для всех игроков
    let pSpot = spots.pop();
    
    // Находим точку для монстра подальше от игроков (>10 блоков)
    ...
    // Подсчет голосов и случайный выбор при ничьей
    let maxV = -1;
    for(let k in votes) { if(votes[k] > maxV) maxV = votes[k]; }
    
    let winners = [];
    for(let k in votes) { if(votes[k] === maxV) winners.push(k); }
    // Если никто не голосовал (все 0) или ничья, рандомим среди победителей
    ...
    for (let id in players) {
        ...
        players[id].color = colors[idx % colors.length]; // Раздаем разные цвета
        players[id].isRunning = false;
        idx++;
    }

    // Первый игрок будет обрабатывать ИИ монстра для сервера
...
io.on('connection', (socket) => {
    socket.on('joinLobby', (data) => {
        ...
        io.emit('voteUpdate', votes); // Передаем текущие голоса новому игроку
    ...
    socket.on('disconnect', () => {
        if (players[socket.id]) {
            ...
            // Убираем голос, если игрок вышел (по желанию, оставим для истории)
