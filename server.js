...
function startGameFlow(lobbyId) {
    ...
    if (l.mode === 'monster') {
        ...
        setTimeout(() => { l.state = 'voting_diff'; io.to(lobbyId).emit('voteDiffStart'); startTimer(lobbyId, 20, () => finalizeGame(lobbyId)); }, 20000);
    } else {
        l.state = 'voting_diff'; io.to(lobbyId).emit('voteDiffStart'); startTimer(lobbyId, 20, () => finalizeGame(lobbyId));
    }
}
...
function finalizeGame(lobbyId) {
    ...
    let exitSpot = spots.pop(); let pSpot = spots.pop(); let mSpot = spots.find(s => Math.hypot(s.x - pSpot.x, s.z - pSpot.z) > 6) || spots.pop();
...
io.on('connection', (socket) => {
    ...
    socket.on('createLobby', data => {
        ...
        socket.emit('lobbiesList', Object.values(lobbies).map(l => ({ id: l.id, name: l.name, hasPass: !!l.password, max: l.max, count: Object.keys(l.players).length })));
    });
    ...
    socket.on('hostStartVoteMode', () => { let l = lobbies[socket.lobbyId]; if(l && l.host === socket.id && l.state === 'waiting' && Object.keys(l.players).length >= 2) { l.state = 'voting_mode'; io.to(l.id).emit('voteModeStart'); startTimer(l.id, 15, () => startGameFlow(l.id)); } });
    ...
    socket.on('requestMonsterTp', targetId => {
        ...
        for(let z=1; z<MAP_SIZE-1; z++) for(let x=1; x<MAP_SIZE-1; x++) if(l.mapData[z][x] === 0) { let dist = Math.hypot(x*BLOCK_SIZE - px, z*BLOCK_SIZE - pz); if(dist >= 8 && dist <= 12) validSpots.push({x: x*BLOCK_SIZE, z: z*BLOCK_SIZE}); }
