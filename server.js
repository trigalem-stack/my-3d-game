...
// --- ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ДЛЯ A* ПОИСКА ПУТИ И ИИ (СЕРВЕР) ---
...
function astar(start, goal, mapData, maxNodes = 2000) {
    const key = (n) => `${n.x},${n.z}`;
...
function serverHasLineOfSight(x0, z0, x1, z1, mapData) {
    ...
    let err = dx - dz;
    while(true) {
        if(gx0 < 0 || gx0 >= MAP_SIZE || gz0 < 0 || gz0 >= MAP_SIZE || mapData[gz0][gx0] === 1) return false;
...
function serverTeleportMonsterNear(room) {
    let validSpots = [];
    let mapData = room.mapData;
    let players = Object.values(room.players).filter(p => !p.isDead);
    
    for(let z=1; z<MAP_SIZE-1; z++) {
        for(let x=1; x<MAP_SIZE-1; x++) {
            if(mapData[z][x] === 0) {
                let wx = x * BLOCK_SIZE; let wz = z * BLOCK_SIZE;
                let valid = false;
                for (let p of players) {
                    let dist = Math.hypot(wx - p.x, wz - p.z);
                    if (dist >= 12 && dist <= 20 && !serverHasLineOfSight(wx, wz, p.x, p.z, mapData)) {
                        valid = true; break;
                    }
                }
                if (valid) validSpots.push({x, z});
            }
        }
    }
    if(validSpots.length === 0) {
        for(let z=1; z<MAP_SIZE-1; z++) {
            for(let x=1; x<MAP_SIZE-1; x++) {
                if(mapData[z][x] === 0) validSpots.push({x, z});
            }
        }
    }
    if(validSpots.length > 0) {
    ...
    room.monster.path = [];
    room.monster.lastKnownPlayer = null;
    room.monster.wanderTarget = null;
...
function finalizeAndStartMatch(room) {
    ...
    room.monster = {
        ...
        wanderTarget: null,
        targetX: mSpot.x * BLOCK_SIZE,
        targetZ: mSpot.z * BLOCK_SIZE,
        path: [],
        pathIndex: 0
    };
    ...
    io.to(room.id).emit('gameStarted', {
        ...
        monsterSpeed: diffStats[room.difficulty].c 
    });
...
function startMatchLoops(room) {
    ...
    room.matchLogicInterval = setInterval(() => {
        ...
        // --- УМНЫЙ ИИ ДЛЯ ОНЛАЙНА ---
        if (room.chosenMode === 'normal') {
            room.monster.stuckTimer -= dt;
            let target = null;
            let minDist = Infinity;
            let seesAnyone = false;
            let stats = diffStats[room.difficulty];

            // 1. Поиск ближайшего игрока в поле зрения или по звуку
            for(let id in room.players) {
                let p = room.players[id];
                if(!p.isDead) {
                    let d = Math.hypot(p.x - room.monster.x, p.z - room.monster.z);
                    let los = serverHasLineOfSight(room.monster.x, room.monster.z, p.x, p.z, room.mapData) && (room.monster.stuckTimer <= 0);
                    let hears = (p.isRunning && d < 15.0);

                    if (los || hears || (d < BLOCK_SIZE * 1.5)) {
                        if (d < minDist) {
                            minDist = d;
                            target = p;
                            seesAnyone = true;
            ...
            let isChasing = seesAnyone;
            if (isChasing) {
                room.monster.lastKnownPlayer = worldToGrid(target.x, target.z);
                room.monster.tpTimer = 19.0;
            } else {
                room.monster.tpTimer -= dt;
                if (room.monster.tpTimer <= 0) {
                    serverTeleportMonsterNear(room);
                    room.monster.tpTimer = 19.0;
                }
            }

            room.monster.repathTimer -= dt;
            let mGrid = worldToGrid(room.monster.x, room.monster.z);
            let directChase = false;

            if (seesAnyone) {
                room.monster.targetX = target.x;
                room.monster.targetZ = target.z;
                directChase = true;
                room.monster.path = [];
            } else if (room.monster.lastKnownPlayer) {
                if (room.monster.path.length === 0 || room.monster.repathTimer <= 0 || room.monster.pathIndex >= room.monster.path.length) {
                    let p = astar(mGrid, room.monster.lastKnownPlayer, room.mapData, 2000);
                    if (p && p.length > 0) { room.monster.path = p; room.monster.pathIndex = 0; } 
                    else { room.monster.lastKnownPlayer = null; }
                    room.monster.repathTimer = 0.5;
                }
                if (room.monster.lastKnownPlayer && mGrid.x === room.monster.lastKnownPlayer.x && mGrid.z === room.monster.lastKnownPlayer.z) {
                    if (!isChasing) { room.monster.lastKnownPlayer = null; room.monster.path = []; }
                }
            } else {
                if (!room.monster.wanderTarget || Math.hypot(room.monster.wanderTarget.x*BLOCK_SIZE - room.monster.x, room.monster.wanderTarget.z*BLOCK_SIZE - room.monster.z) < 1.0 || !room.monster.path || room.monster.path.length === 0 || room.monster.pathIndex >= room.monster.path.length) {
                    let spots = getEmptySpots(room.mapData);
                    if (spots.length > 0) {
                        room.monster.wanderTarget = spots[Math.floor(Math.random() * spots.length)];
                        let p = astar(mGrid, room.monster.wanderTarget, room.mapData, 2000);
                        if (p && p.length > 0) { room.monster.path = p; room.monster.pathIndex = 0; }
                        else { room.monster.path = []; }
                    }
                }
            }

            if (!directChase && room.monster.path && room.monster.pathIndex < room.monster.path.length) {
                let nextNode = room.monster.path[room.monster.pathIndex];
                room.monster.targetX = nextNode.x * BLOCK_SIZE;
                room.monster.targetZ = nextNode.z * BLOCK_SIZE;
                if (Math.hypot(room.monster.targetX - room.monster.x, room.monster.targetZ - room.monster.z) < 0.4) {
                    room.monster.pathIndex++;
                    if(room.monster.pathIndex < room.monster.path.length) {
                        nextNode = room.monster.path[room.monster.pathIndex];
                        room.monster.targetX = nextNode.x * BLOCK_SIZE;
                        room.monster.targetZ = nextNode.z * BLOCK_SIZE;
                    }
                }
            ...
            // Перемещение с проверкой коллизий (Анти-застревание)
            let mdx = room.monster.targetX - room.monster.x;
            let mdz = room.monster.targetZ - room.monster.z;
            let distToTarget = Math.hypot(mdx, mdz);
            let movedX = false, movedZ = false;

            if (distToTarget > 0.05) {
                let speed = seesAnyone ? stats.c : stats.w;
                let moveDist = speed * dt;
                
                let ndx = mdx / distToTarget;
                let ndz = mdz / distToTarget;
                let moveX = ndx * moveDist;
                let moveZ = ndz * moveDist;
                let botHitbox = 0.25;

                if (!checkServerWallCollision(room.mapData, room.monster.x + moveX, room.monster.z, botHitbox)) {
                    room.monster.x += moveX; movedX = true;
                }
                if (!checkServerWallCollision(room.mapData, room.monster.x, room.monster.z + moveZ, botHitbox)) {
                    room.monster.z += moveZ; movedZ = true;
                }

                if (!movedX && !movedZ) { room.monster.stuckTimer = 0.5; room.monster.repathTimer = 0; }
                if (movedX || movedZ) { room.monster.dirX = ndx; room.monster.dirZ = ndz; }
                else if (!directChase && room.monster.path) { room.monster.repathTimer = 0; }
            }

            io.to(room.id).emit('monsterUpdate', room.monster);

            // Убивает ЛЮБОГО игрока на своем пути, даже если просто проходил мимо
            let killOccurred = false;
            for(let id in room.players) {
                let p = room.players[id];
                if(!p.isDead && Math.hypot(p.x - room.monster.x, p.z - room.monster.z) < 1.3) {
                    p.isDead = true;
                    io.to(room.id).emit('playerDiedEvent', p.id);
                    if (room.monster.lastKnownPlayer && p.x === target?.x) room.monster.lastKnownPlayer = null; 
                    killOccurred = true;
                }
            }
            if (killOccurred) { checkMatchEndConditions(room); }
        }
