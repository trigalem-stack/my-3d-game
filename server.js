...
function startMatchLoops(room) {
    ...
    // Server Tick = 100ms (ускорено для плавной симуляции ИИ при пинге)
    const tickMs = 100;
    ...
    room.matchLogicInterval = setInterval(() => {
        ...
        // Плавный A* ИИ ДЛЯ ОБЫЧНОГО РЕЖИМА (Сглаженный пинг)
        if (room.chosenMode === 'normal') {
            ...
            if (target) {
                ...
                // Проверка на убийство ЛЮБОГО игрока на пути, а не только цели
                ...
                for(let id in room.players) {
                    ...
                    if(!p.isDead && Math.hypot(p.x - room.monster.x, p.z - room.monster.z) < 1.3) {
                        ...
                        if (room.monster.targetId === id) room.monster.targetId = null; // Сброс цели
                        killOccurred = true;
                    }
                }
                if (killOccurred) {
...
io.on('connection', (socket) => {
    ...
    socket.on('monsterMove', (data) => {
        ...
        if (room && room.gameState === 'playing' && room.monsterId === socket.id) {
            ...
            // ФИКС: Обязательно обновлять координаты игрока-монстра на сервере, чтобы его ник двигался с ним!
            room.players[socket.id].x = data.x;
            room.players[socket.id].z = data.z;
